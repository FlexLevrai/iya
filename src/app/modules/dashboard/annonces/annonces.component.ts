...
export class AnnoncesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.annonceService.annonceList(this.email).then((data:Annonce[]) => {
      this.annonces = data;
    }).catch((error) =>{
    });
  ...
  onSubmit() {
    ...
    const nombre =  this.annonceForm.get('nombre').value;
    const adresse =  this.annonceForm.get('adresse').value;
    ...
    this.annonceService.sendAnnonce(annonce).then((data: Annonce) => {
  ...
  onCandidate(id:number){
    this.router.navigate(['/dashboard', 'candidats',  id ]);
  }
  onDelete(id:number){
